...
def normalize_object_addr(value: Any) -> str:
    """
    Нормализация адреса объекта.
    Агрессивные регулярные выражения (удаление/добавление пробелов вокруг дефисов,
    запятых и слешей) убраны, чтобы не ломать точное строковое совпадение 
    со справочником объектов из базы данных.
    """
    # Базовая очистка: убираем двойные пробелы и пробелы по краям
    s = normalize_spaces(str(value or ""))

    # Унифицируем разные виды тире в обычный минус (это безопасно)
