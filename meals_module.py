...
class MealOrderPage(tk.Frame):
    ...
    def _update_date_hint(self):
        try:
            ...
            tomorrow = date.today() + timedelta(days=1)
            if req is None:
                self.lbl_date_hint.config(text="Укажите дату в формате YYYY-MM-DD", fg="#b00020")
            elif req != tomorrow:
                self.lbl_date_hint.config(text=f"Заявка возможна только на {tomorrow.strftime('%Y-%m-%d')}", fg="#b00020")
            else:
                self.lbl_date_hint.config(text="✓ Заявка на завтрашнюю дату", fg="#2e7d32")
...
class MealPlanningPage(tk.Frame):
    ...
    def _show_details_dialog(self, entry: Dict):
        ...
        header = tk.Frame(dialog, bg="#e3f2fd", relief="solid", borderwidth=1)
        header.pack(fill="x", padx=0, pady=0)
        tk.Label(header, text=f"📅 Дата: {entry.get('date', '')} | 📍 {entry.get('address', '')}", 
                 font=("Arial", 12, "bold"), bg="#e3f2fd", fg="#0066cc", padx=15, pady=12).pack(anchor="w")

        info_frame = tk.Frame(dialog, bg="#f7f7f7")
        info_frame.pack(fill="x", padx=15, pady=10)
        tk.Label(info_frame, text=f"Всего заявок: {entry.get('total_count', 0)} человек", 
                 font=("Arial", 11, "bold"), bg="#f7f7f7").pack(anchor="w")
