...
# Основная логика:
# - Ищем лист, где в шапке встречается «ЛОКАЛЬНАЯ СМЕТА».
...
#     • Материалы: строки 3-й колонки с «МР», «материалы», «мат.», «материал…».
#     • Inline-материалы: основная строка позиции (3-я колонка — название) с ненулевой стоимостью в 11/10 кол.,
#       и при этом ед. изм. не «%» и не трудочасы (чел-ч). Такие строки считаем материалами.
# - Прочие = Итого − Материалы − Заработная плата (служебные строки — игнор).
# - Есть общий режим (generic) для CSV/XLSX-таблиц без сметной структуры: ручное сопоставление колонок.
...
class BudgetAnalysisPage(tk.Frame):
    ...
    def __init__(self, master):
        ...
        hint = tk.Label(
            self,
            text=("Поддержка Smeta.RU: лист «ЛОКАЛЬНАЯ СМЕТА», 11 колонок.\n"
                  "Суммы — из 11-й (приоритет) или 10-й колонки «ВСЕГО». "
                  "ЗП/в т.ч. ЗПМ → «Заработная плата», МР/Материалы/Мат. → «Материалы». "
                  "Inline-материалы: если в строке наименования есть стоимость в 11/10, а ед. изм. не «%»/не трудочасы."),
    ...
    def _classify_row(self, row: List[Any]) -> Optional[str]:
        ...
        Возвращает категорию строки:
          ...
          - 'materials' — МР/Материалы/Мат. ИЛИ строка позиции с прямой стоимостью в 11/10 кол. (inline-материал),
                          при этом ед. изм. не «%» и не трудочасы.
        ...
        # Inline-материалы: есть стоимость в 11/10 кол., ед. изм. не % и не чел-ч
