...
class TimesheetComparePage(tk.Frame):
    ...
    def _build_ui(self):
        ...
        hdr.pack(fill="x")
        tk.Label(
            hdr, text="🔍  Сравнение табелей (Объекты + Командировки vs 1С)",
        ...
        tk.Label(filter_pnl, text="Год:", font=("Segoe UI", 9), bg=CMP_COLORS["panel"]
        ...
        tk.Label(filter_pnl, text="Месяц:", font=("Segoe UI", 9), bg=CMP_COLORS["panel"]
        ...
        tk.Label(filter_pnl, text="Подразделение:", font=("Segoe UI", 9), bg=CMP_COLORS["panel"]
        ...
        ttk.Button(btn_f, text="Сбросить", command=self._reset_filters).pack(side="left", padx=(0, 4))

        tk.Button(
            btn_f, text="📂 Загрузить 1С (xlsx)…",
            font=("Segoe UI", 9, "bold"), bg="#2e7d32", fg="white",
        ...
        sel_pnl = tk.LabelFrame(
            self, text=" 1️⃣  Выбор табеля для сравнения ",
            font=("Segoe UI", 9, "bold"), bg=CMP_COLORS["panel"], fg=CMP_COLORS["accent"],
            relief="groove", bd=1
        )
        ...
        tk.Label(sel_top, text="Двойной щелчок или кнопка «Сравнить» — загрузить данные:",
                 font=("Segoe UI", 8), fg="#555", bg=CMP_COLORS["panel"]).pack(side="left")

        tk.Button(
            sel_top, text="▶  Сравнить выбранное",
            font=("Segoe UI", 9, "bold"), bg=CMP_COLORS["btn_save_bg"], fg=CMP_COLORS["btn_save_fg"],
            activebackground="#0d47a1", activeforeground="white",
            relief="flat", cursor="hand2", padx=10, pady=3,
        ...
        self.tree_headers = ttk.Treeview(hdr_tbl, columns=cols_h, show="headings", height=4, selectmode="browse")
        ...
        cmp_pnl = tk.LabelFrame(
            cmp_outer, text=" 2️⃣  Результат сравнения ",
            font=("Segoe UI", 9, "bold"), bg=CMP_COLORS["panel"], fg=CMP_COLORS["accent"],
            relief="groove", bd=1
        )
        ...
        tk.Label(
            cmp_tool, text="Период:", font=("Segoe UI", 9, "bold"),
            bg=CMP_COLORS["accent_light"], fg=CMP_COLORS["accent"]
        ...
        ttk.Checkbutton(
            cmp_tool, text="Только расхождения", variable=self.var_only_diff,
        ...
        for color, label in [
            ("#ffcdd2", "Расхождение"), ("#ffe0b2", "Только в 1С"),
            ("#fff9c4", "Только в объекте"), ("#f5f5f5", "Строка 1С")
        ]:
            dot = tk.Label(cmp_tool, text="  ", bg=color, relief="solid", bd=1)
            dot.pack(side="left", padx=(4, 2))
            tk.Label(cmp_tool, text=label, font=("Segoe UI", 8), bg=CMP_COLORS["accent_light"]
                     ).pack(side="left", padx=(0, 6))

        self.lbl_stat = tk.Label(cmp_tool, text="", font=("Segoe UI", 9, "bold"),
                                 fg=CMP_COLORS["accent"], bg=CMP_COLORS["accent_light"])
        ...
        tk.Label(status_bar, textvariable=self.var_status, anchor="w", font=("Segoe UI", 9),
                 bg=CMP_COLORS["panel"]).pack(side="left", padx=6)

        self.progress_bar = ttk.Progressbar(status_bar, orient="horizontal", mode="determinate", length=240)
    ...
    def _update_stat_label(self):
        ...
        if self._stat_diff: parts.append(f"⚠ Расхождений: {self._stat_diff}")
        if self._stat_only_obj: parts.append(f"Только объект: {self._stat_only_obj}")
        if self._stat_only_1c: parts.append(f"Только 1С: {self._stat_only_1c}")
    ...
    def _load_headers(self):
        ...
        self._agg_headers = sorted(agg_map.values(), key=lambda a: (a["year"], a["month"], a["department"]), reverse=True)

        for agg in self._agg_headers:
            iid = f"{agg['year']}:{agg['month']}:{agg['department']}"
            self.tree_headers.insert("", "end", iid=iid, values=(agg["year"], month_name_ru(agg["month"]), agg["department"], len(agg["headers"])))
    ...
    def _on_select_header(self):
        ...
        agg = next((a for a in self._agg_headers if f"{a['year']}:{a['month']}:{a['department']}" == sel[0]), None)
        if not agg: return
        ...
        obj_rows = []
        try:
            # 1. Загрузка стандартных табелей
            ...
            if DB_POOL:
                ...
                try:
                    with conn.cursor() as cur:
                        ...
                        sql = f"""
                            SELECT th.object_addr, th.object_id, tr.fio, tr.tbn, tr.hours_raw
                            ...
                            WHERE th.year = %s AND th.month = %s {dep_sql}
                        """
                    ...
                    logging.exception("Error loading trip timesheets")
                finally:
    ...
    def _sum_days(self, days: List, count: int) -> float:
        ...
        return sum(get_number_value(d) or 0.0 for d in days[:count])
    ...
    def _render_compare_from_groups(self):
        ...
        for grp in self._merged_groups:
            ...
            tag_obj = {"only_obj": "only_obj", "only_1c": "only_1c"}.get(situation, "obj_diff" if has_diff else "obj_ok")
            icon    = {"only_obj": "⚪", "only_1c": "🟠"}.get(situation, "🔴" if has_diff else "🟢")
            ...
            for o_row in obj_rows:
                vals = [icon if first else "", grp["display_fio"] if first else "", grp["display_tbn"] if first else "", o_row["object_display"], "Объект"]
                ...
                vals += day_vals + [_fmt(sum_obj) if first else "", _fmt(sum_1c) if first else ""]
            ...
            if hr_row:
                ...
                self.tree_compare.insert("", "end",
                    values=(["", grp["display_fio"], grp["display_tbn"], "", "1С Кадры"] + hr_days_disp + ["", _fmt(sum_1c)]),
                    tags=("only_1c" if situation == "only_1c" else "hr_row",))
    ...
    def _rebuild_comparison(self):
        ...
        def check_has_data_in_period(days_list):
            for i in range(days_count):
                if i < len(days_list) and normalize_text(days_list[i]): return True
            return False

        for tbn in sorted(set(hr_map) | set(obj_map)):
            hr_row = hr_map.get(tbn)
            obj_rows_lst = sorted(obj_map.get(tbn, []), key=lambda x: x.get("object_display", ""))

            has_data = False
            if hr_row and check_has_data_in_period(hr_row["days"]): has_data = True
            if not has_data:
            ...
            if has_obj and not has_1c: situation = "only_obj"
            elif has_1c and not has_obj: situation = "only_1c"
            else: situation = "both"
            ...
            for i in range(days_count):
                ...
                if has_num:
                    merged_obj_days.append(str(int(num_sum)) if num_sum.is_integer() else str(num_sum).rstrip("0").rstrip("."))
                else:
            ...
            for i in range(days_count):
                ...
                n = get_number_value(v)
                if n is not None:
                    merged_hr_days.append(str(int(n)) if n.is_integer() else str(n).rstrip("0").rstrip("."))
                else:
            ...
            stat_total += 1
            if situation == "only_obj": stat_only_obj += 1
            elif situation == "only_1c": stat_only_1c += 1
            elif group_has_diff: stat_diff += 1
        ...
        self._merged_groups.sort(key=lambda g: (fio_sort_key(g.get("display_fio")), normalize_tbn(g.get("display_tbn"))))
        self._render_compare_from_groups()        
        
        self._stat_total, self._stat_diff, self._stat_only_obj, self._stat_only_1c = stat_total, stat_diff, stat_only_obj, stat_only_1c
        self._update_stat_label()

        self.var_status.set(f"Сравнение завершено — сотрудников: {stat_total} | расхождений: {stat_diff} | только объект: {stat_only_obj} | только 1С: {stat_only_1c}")
    ...
    def _start_export_thread(self):
        if not self._merged_groups: return messagebox.showwarning("Экспорт", "Нет данных для экспорта.", parent=self)

        fpath = filedialog.asksaveasfilename(
            title="Сохранить", defaultextension=".xlsx",
    ...
    def _export_process(self, fpath: str):
        try:
            ...
            ws.append([f"Сравнение табелей (вкл. командировки). Период: {self.var_compare_mode.get()}. Экспорт: {datetime.now().strftime('%d.%m.%Y %H:%M')}"])
            ws.append([f"Сотрудников: {self._stat_total} | Расхождений: {self._stat_diff} | Только объект: {self._stat_only_obj} | Только 1С: {self._stat_only_1c}"])
            ws.append([])

            hdr_row = ["Статус", "ФИО", "Таб.№", "Объект", "Источник"] + [str(i) for i in range(1, days_cnt + 1)] + ["∑ Объект", "∑ 1С"]
            ws.append(hdr_row)
            ...
            border_all_thin = Border(left=Side(style="thin", color="A0A0A0"), right=Side(style="thin", color="A0A0A0"), top=Side(style="thin", color="A0A0A0"), bottom=Side(style="thin", color="A0A0A0"))
            align_center = Alignment(horizontal="center", vertical="center")

            for c in range(1, len(hdr_row) + 1):
                cell = ws.cell(hdr_excel_row, c)
                cell.font, cell.fill, cell.alignment, cell.border = font_header, fill_header, align_center, border_all_thin

            total_grp = len(self._merged_groups)
            
            def _to_excel_val(val):
                if val is None or str(val).strip() == "None" or str(val).strip() == "": return ""
                try:
            ...
            for idx, grp in enumerate(self._merged_groups):
                self.after(0, lambda v=int(idx/total_grp*100) if total_grp else 0: self.progress_bar.configure(value=v))
                
                situation, has_diff, hr_row, obj_rows, is_diff = grp.get("situation"), grp.get("has_diff"), grp.get("hr_row"), grp.get("obj_rows", []), grp.get("daily_diffs", [])
                icon = "⚪ Только объект" if situation == "only_obj" else "🟠 Только 1С" if situation == "only_1c" else "🔴 Расхождение" if has_diff else "🟢 OK"
                ...
                for o_row in obj_rows:
                    ...
                    row_data = ([icon if first else "", grp["display_fio"] if first else "", grp["display_tbn"] if first else "", o_row["object_display"], "Объект"]
                                + [_to_excel_val(v) for v in days_raw] + [_to_excel_val(sum_obj) if first else "", _to_excel_val(sum_1c) if first else ""])
                    ws.append(row_data)
                    cur = ws.max_row
                    ...
                    for c_idx in range(1, len(row_data) + 1):
                        ...
                        if c_idx >= 6: cell.alignment = align_center

                    if situation == "both":
                        for i in range(days_cnt):
                            if i < len(is_diff) and is_diff[i] and str(days_raw[i]).strip() and str(days_raw[i]) != 'None':
                                c = ws.cell(cur, 6 + i)
                                c.fill, c.font = fill_cell_diff, font_bold

                    if situation == "only_obj":
                        for c_idx in range(1, len(row_data) + 1): ws.cell(cur, c_idx).fill = fill_only_obj
                    elif situation == "both" and has_diff:
                        for c_idx in range(1, 6): ws.cell(cur, c_idx).fill = fill_diff
                    first = False

                if hr_row:
                    hr_days = (hr_row["days"][:days_cnt] + [None] * days_cnt)[:days_cnt]
                    row_data = (["", grp["display_fio"], grp["display_tbn"], "", "1С Кадры"] + [_to_excel_val(v) for v in hr_days] + ["", _to_excel_val(sum_1c)])
                    ws.append(row_data)
                    cur = ws.max_row
                    ...
                    for c_idx in range(1, len(row_data) + 1):
                        cell = ws.cell(cur, c_idx)
                        cell.fill, cell.border = fill_1c, border_all_thin
                        if c_idx >= 6: cell.alignment = align_center
                ...
                if end_row >= start_row:
                    for c_idx in range(1, len(hdr_row) + 1):
                        ws.cell(start_row, c_idx).border = Border(left=ws.cell(start_row, c_idx).border.left, right=ws.cell(start_row, c_idx).border.right, top=Side(style="medium"), bottom=ws.cell(start_row, c_idx).border.bottom)
                        ws.cell(end_row, c_idx).border = Border(left=ws.cell(end_row, c_idx).border.left, right=ws.cell(end_row, c_idx).border.right, top=ws.cell(end_row, c_idx).border.top, bottom=Side(style="medium"))
                    if end_row > start_row:
                        for col_n in (1, 2, 3):
                            ws.merge_cells(start_row=start_row, start_column=col_n, end_row=end_row, end_column=col_n)
                            ws.cell(start_row, col_n).alignment = Alignment(horizontal="left" if col_n == 2 else "center", vertical="center", wrap_text=True)

            ws.column_dimensions["A"].width, ws.column_dimensions["B"].width, ws.column_dimensions["C"].width, ws.column_dimensions["D"].width, ws.column_dimensions["E"].width = 16, 34, 10, 44, 12
            for i in range(days_cnt): ws.column_dimensions[get_column_letter(6 + i)].width = 4.5
            ws.column_dimensions[get_column_letter(6 + days_cnt)].width = ws.column_dimensions[get_column_letter(7 + days_cnt)].width = 9.5
